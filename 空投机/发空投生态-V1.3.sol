...
contract anyAirdrop{
    ...
    function giveAirdrop(address _tokenAdr,uint _AirdropAmount , uint _AirdropTotal) public{
        ...
        tokenMap[_tokenAdr] = AirToken(_tokenAdr, _AirdropAmount, msg.sender, _a , 1);
    ...
    uint public Counting ; //防刷记数
    uint public timeRecording; //防刷记时
    uint public _timeRecording = 60 ; //几分钟内
    uint public _Counting = 5 ; //几笔
    uint a = 1;
    
    function getTimeRecording()public view returns(uint){
        if(now > timeRecording.add( (_timeRecording.mul(1 minutes)) )){
            return a;
        }
    }
    ...
    function getAirdrop(address _tokenAdr) public{
        ...
        if(_Airtoken1.defense == 1){
            ...
            require(Counting <= _Counting ,'Defense triggered');
        }
        //////////////////////////////////////////////////////////////////
        require(_Airtoken1.isgetmap[msg.sender] != tokenMap[_tokenAdr].a);
    ...
    function claim(address _tokenAdr) public{
        ...
        tokenMap[_tokenAdr]= AirToken(address(0), 0, address(0),tokenMap[_tokenAdr].a , 1);
    }
    ...
    //空投发布人可开启或关闭防刷 1为开，0为关
    function ONoffDefense(address _tokenAdr, uint _defense) public{
        AirToken storage _Airtoken1 = tokenMap[_tokenAdr];
        require(_Airtoken1.tokenOwner == msg.sender);
        _Airtoken1.defense = _defense;
    }
    
    //获取空投信息
    function getAirTokenInfo(address _tokenAdr) public view returns(address,uint,address,uint, uint){
        AirToken storage _Airtoken1 = tokenMap[_tokenAdr];
        return (_Airtoken1.tokenAdr, _Airtoken1.AirdropAmount, _Airtoken1.tokenOwner ,_Airtoken1.a , _Airtoken1.defense);
    }
...
/////////////////////////////////////////////////////////////
    ...
    //管理员设置几分钟内几笔交易触发防刷
    function setDefense(uint __timeRecording, uint __Counting)public ownerOnly{
        _timeRecording = __timeRecording;
        _Counting = __Counting ;
    }
//////////////////////////////////////////////////////////////
