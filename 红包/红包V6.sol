...
contract hongbao{
    ...
    mapping(uint => HBinfo)public HBmap; //红包信息与ID绑定

    mapping(address => uint) public  feeMap; //记录有多少是LG的收益
    mapping(address => string)public userName; //用户昵称存储
    ...
    //用ID号获取红包信息
    function getHBinfo(uint _HBid)public view returns(uint, string,string,address,uint,uint,uint,uint,uint, uint){
    ...
    function giveTokenHB(address _tokenAdr, uint _total, uint _amount, string _remark, uint _lownBalance,uint _password ) public payable{
        require(Itoken(_tokenAdr).balanceOf(msg.sender) >= _total && _total >= 10000 && _amount >=1  && Itoken(LGadr).balanceOf(msg.sender) >= lowLG); //要求发送者拥有足够的TOKEN,总量大于1万（防计算溢出）,使用本系统最少持有lowLGLG
        if(tokenIsPayFee[_tokenAdr]==1){require(msg.value==HTforFee); }
        uint _HBid;//用在for循环里
        uint _HBid2;
        
        //寻找已有的红包ID中有没有空的ID 
        for(uint i=1; _HBid==0 && i<=HBid ; i++){  
            if(HBmap[i].id == 0){
                _HBid = i;
            }
        }
    ...
    //领红包
    function getHB(uint _HBid, uint _password) public LGCchack{
        HBinfo storage _HBinfo = HBmap[_HBid];
        require(_HBinfo.isgetmap[msg.sender] != _HBinfo.time && Itoken(LGadr).balanceOf(msg.sender) >= lowLG );
    ...
    //24小时后发红包者可领取未领完红包,管理员可任何时间退回红包并扣取手续费
    function getHBbalance(uint _HBid)public{
        HBinfo storage _HBinfo = HBmap[_HBid];
        if(msg.sender != owner){
             require(_HBinfo.userAdr == msg.sender && now >= _HBinfo.time.add(overtime.mul(1 hours)));
        }
        ...
        Itoken(_HBinfo.tokenAdr).transfer(_HBinfo.userAdr, _HBinfo._balance);
        delete HBmap[_HBid];
    }
    ...
    uint public overtime = 24; //默认24小时
    uint public LGCamount = 5;  //要求多少信用分可使用
    

    constructor(address _LGadr,address _LGC, address _ILGVgongxianAddr) public {
        owner = msg.sender;
        LGadr = _LGadr ;
        LGC = _LGC;
    ...
    modifier LGCchack() {
        require(Itoken(LGC).balanceOf(msg.sender) >=LGCamount);
        _; 
    }
    ...
    //设置超时领回(默认24小时)
        function setOvertime(uint _overtime) public ownerOnly{
        overtime = _overtime;
    }
